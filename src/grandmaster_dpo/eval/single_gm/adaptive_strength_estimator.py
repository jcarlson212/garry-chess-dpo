...
def _maia2_pick_move_uci(
    ...
    """
    If tuned_model_depth > 0: use your depth-limited Maia beam search (deterministic-ish).
...
def _make_stockfish(
    ...
    *,
    ...
    if "Hash" in eng.options:
        cfg["Hash"] = int(hash_mb)
    if "Ponder" in eng.options:
        cfg["Ponder"] = False
    try:
        if cfg:
            eng.configure(cfg)
...
def _stockfish_pick_move_fixed(
    ...
    *,
    ...
    if nodes is not None and nodes > 0:
        ...
        # depth must be present for depth-based play
...
def _worker_init(state: Dict[str, Any]) -> None:
    ...
    # One Stockfish engine per worker (anchors differ only by Limit depth/nodes)
...
def _build_gauntlet_tasks(
    *,
    ...
    """
    Task: (task_id, model_key, anchor_key, n_games)
    Within task we alternate colors so each match is balanced.
    """
...
def _run_gauntlet_task(task: Tuple[int, str, str, int]) -> List[Tuple[str, str, float, str, str]]:
    """
    Returns list of (white_key, black_key, score_white, result_str, pgn_text).
    Alternates colors each game for balance: model starts white on even g.
    """
...
def main() -> None:
    ...
    if use_nodes:
        for n in sf_nodes_list:
            ...
            player_specs.append(PlayerSpec(
                key=k, kind="sf", param=f"nodes={n}", sf_nodes=n, sf_depth=None
            ))
    else:
        for d in sf_depth_list:
            ...
            anchor_keys.append(k)
            player_specs.append(PlayerSpec(
                key=k, kind="sf", param=f"depth={d}", sf_depth=d, sf_nodes=None
            ))
    ...
    # We'll stream-write PGNs & CSV as results arrive (avoid huge memory).
        ...
        with ctx.Pool(processes=workers, initializer=_worker_init, initargs=(init_state,)) as pool:
            for out in pool.imap_unordered(_run_gauntlet_task, tasks):
                for wk, bk, s_w, res, pgn_text in out:
                    # write PGN (blank line between games)
                    pgn_f.write(pgn_text)
                    pgn_f.write("\n\n")

                    # determine which is model/anchor for bookkeeping
                    # (model_keys are Maia; anchors are Stockfish)
                    if wk in model_keys:
                        model_key, anchor_key = wk, bk
                        score_white = s_w
                    elif bk in model_keys:
                        model_key, anchor_key = bk, wk
                        score_white = s_w  # still score for "white", independent of which is model
                    else:
    ...
    # Optional quick Elo fit (anchored). Real recommendation: ordo/BayesElo on the PGN.
    if args.fit_elos:
        ...
        for ps in sorted(player_specs, key=lambda p: elos[p.key], reverse=True):
            print(f"{ps.key:22s} {ps.kind:6s} {ps.param:12s} elo={elos[ps.key]:7.1f}")
        print(f"Wrote: {out_elo_csv}")

    # Friendly reminder on next step
