...
class KasparovNetGUI:
    ...
    def maybe_make_model_move(self) -> None:
        ...
        # Run inference in a background thread so UI stays snappy
        t = threading.Thread(target=self._model_move_worker, daemon=True)
        t.start()

    def _model_move_worker(self) -> None:
        try:
            fen = self.board.fen()
            ...
            best = pick_best_move(move_probs)

            # prepare side panel text
            top = topk_from_move_probs(move_probs, self.topk)
            lines = []
            lines.append(f"win_prob (white POV): {float(win_prob):.4f}" if win_prob is not None else "win_prob: None")
            lines.append("")
            for uci, p in top:
                lines.append(f"{uci:6s}   p={p:.4f}")
            text = "\n".join(lines)

            # apply back on main thread
            self.root.after(0, lambda: self._apply_model_move(best, text))
        except Exception as e:
            self.root.after(0, lambda: self._handle_model_error(e))

    def _apply_model_move(self, best_uci: str, candidates_text: str) -> None:
        self.set_candidates_text(candidates_text)

        try:
            mv = chess.Move.from_uci(best_uci)
        except Exception:
            messagebox.showerror("Model error", f"Model produced invalid move: {best_uci}")
            self.model_busy = False
            self.redraw()
            return
