...
def train_one_run(cfg: TrainConfig) -> None:
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for batch in train_loader:
            ...
            if step % 50 == 0:
                print(f"[epoch {epoch}] step={step} train_sft_and_kpo_loss={running/max(1,seen):.4f}")

        metrics = evaluate(policy, ref, all_moves_dict, elo_dict, val_loader, device=device, beta=cfg.beta, gamma=cfg.gamma)
        val_loss = metrics["loss"]
        print(f"[epoch {epoch}] val_sft_and_kpo_loss={val_loss:.4f}")

        ckpt_path = out_dir / f"policy_epoch{epoch}_sft_and_kpo_{make_run_name(cfg)}.pt"
        torch.save(policy.state_dict(), ckpt_path)
        print(f"Saved: {ckpt_path}")

        if val_loss < best_val:
            best_val = val_loss
            best_path = out_dir / f"policy_best_sft_and_kpo_{make_run_name(cfg)}.pt"
            torch.save(policy.state_dict(), best_path)
            print(f"Saved best: {best_path} (val_sft_and_kpo_loss={best_val:.4f})")

def make_run_name(cfg: TrainConfig) -> str:
    return f"beta={cfg.beta:.2f}_gamma={cfg.gamma:.2f}_kpo_weight={cfg.kpo_weight:.2f}"
    ...
    if args.preset == "single":
        cfg = TrainConfig(
            **base,
            beta=args.beta,
            gamma=args.gamma,
            kpo_weight=args.kpo_weight,
            run_name=f"single_beta={args.beta:.2f}_gamma={args.gamma:.2f}_kpo_weight={args.kpo_weight:.2f}",
        )
        runs.append(cfg)

    elif args.preset == "overnight":
        combos = [
            {"beta": 0.6, "gamma": 0.5, "kpo_weight": 0.02},
            {"beta": 0.6, "gamma": 1.0, "kpo_weight": 0.02},
            {"beta": 0.6, "gamma": 0.5, "kpo_weight": 0.1},
            {"beta": 0.6, "gamma": 1.0, "kpo_weight": 0.1},
        ]
        for d in combos:
            beta = d["beta"]
            gamma = d["gamma"]
            kpo_weight = d["kpo_weight"]
            cfg = TrainConfig(
                **base,
                beta=beta,
                gamma=gamma,
                kpo_weight=kpo_weight,
                run_name=f"beta={beta:.2f}_gamma={gamma:.2f}_kpo_weight={kpo_weight:.2f}",
            )
            runs.append(cfg)
...
def main() -> None:
    # Example usage: python ./src/grandmaster_dpo/train/single_gm/train_sft_and_kpo_maia2.py --gm_name caruana --train_val_folder ./final_experiments_for_paper/experiment1/train_val_pgns_twic --out_dir ./final_experiments_for_paper/experiment1/trained_models_twic --beta 0.8 --gamma 1.2 --preset overnight
    ...
    ap.add_argument("--kpo_weight", type=float, default=0.02)
