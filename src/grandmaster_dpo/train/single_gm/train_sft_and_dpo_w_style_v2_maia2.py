...
def main() -> None:
    # Example usage: python ./src/grandmaster_dpo/train/single_gm/train_sft_and_dpo_w_style_v2_maia2.py --gm_name caruana --train_val_folder ./final_experiments_for_paper/experiment1/train_val_pgns_twic --out_dir ./final_experiments_for_paper/experiment1/trained_models_twic --dpo_loss_weight 0.1
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            if step % 50 == 0:
                print(f"[epoch {epoch}] step={step} train_sft_and_dpo_w_style_v2_loss={running/max(1,seen):.4f}")
        ...
        print(f"[epoch {epoch}] val_sft_and_dpo_w_style_v2_loss={val_loss:.4f}")


        ckpt_path = out_dir / f"policy_epoch{epoch}_sft_and_dpo_w_style_v2_beta={args.beta:.2f}_dpo_loss_weight={args.dpo_loss_weight:.2f}_style_cp_scale={args.style_cp_scale:.2f}_style_piece_bonus={args.style_piece_bonus:.2f}_style_positional_bonus={args.style_positional_bonus:.2f}_style_tau={args.style_tau:.2f}.pt"
        torch.save(policy.state_dict(), ckpt_path)
        print(f"Saved: {ckpt_path}")

        if val_loss < best_val:
            best_val = val_loss
            best_path = out_dir / f"policy_best_sft_and_dpo_w_style_v2_beta={args.beta:.2f}_dpo_loss_weight={args.dpo_loss_weight:.2f}_style_cp_scale={args.style_cp_scale:.2f}_style_piece_bonus={args.style_piece_bonus:.2f}_style_positional_bonus={args.style_positional_bonus:.2f}_style_tau={args.style_tau:.2f}.pt"
            torch.save(policy.state_dict(), best_path)
            print(f"Saved best: {best_path} (val_sft_and_dpo_w_style_v2_loss={best_val:.4f})")
