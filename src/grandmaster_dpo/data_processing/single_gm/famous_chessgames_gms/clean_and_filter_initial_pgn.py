...
def main():
    # Example usage: python ./src/grandmaster_dpo/data_processing/single_gm/famous_chessgames_gms/clean_and_filter_initial_pgn.py  --pgn ./data/raw/chessgames_famous/alekhine_chessgames.pgn --gm_name alekhine
    ...
    out_path = Path(f"./processed/single_gm/famous_chessgames_gms/{args.gm_name}.pgn")
    ...
    games = []
    games.extend(all_games[0:400])
    np.random.shuffle(games)
    print(f"Filtered all games")
    print(f"Total games after filtering (should be 1000): {len(games)}")
